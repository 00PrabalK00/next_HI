...
public:
  ...
  last_sent_time_.resize(info_.joints.size(), std::chrono::steady_clock::time_point::min());
    ...
    const std::string heartbeat_node_name = "kinco_hardware_interface_" + can_interface_;
    heartbeat_node_ = std::make_shared<rclcpp::Node>(heartbeat_node_name);
    motor_heartbeat_pub_ = heartbeat_node_->create_publisher<std_msgs::msg::Bool>("motor_heartbeat", 10);
    RCLCPP_INFO(rclcpp::get_logger("KincoHardwareInterface"), "Publishing motor heartbeat on topic 'motor_heartbeat'");
  ...
  hardware_interface::CallbackReturn on_deactivate(const rclcpp_lifecycle::State & /*previous_state*/) override {
    ...
    motors_all_on_since_ = std::chrono::steady_clock::time_point::min();
    last_heartbeat_publish_time_ = std::chrono::steady_clock::time_point::min();
    return hardware_interface::CallbackReturn::SUCCESS;
  }
  ...
  hardware_interface::CallbackReturn on_cleanup(const rclcpp_lifecycle::State & /*previous_state*/) override {
    ...
    motors_all_on_since_ = std::chrono::steady_clock::time_point::min();
    last_heartbeat_publish_time_ = std::chrono::steady_clock::time_point::min();
    motor_heartbeat_pub_.reset();
    heartbeat_node_.reset();
    return hardware_interface::CallbackReturn::SUCCESS;
  }
  ...
  bool are_all_motors_operation_enabled() const {
    if (statuswords_.empty()) {
      return false;
    }
    for (const auto sw : statuswords_) {
  ...
  void publish_motor_heartbeat() {
    if (!motor_heartbeat_pub_) {
      return;
    }
    const bool all_motors_on =
      current_state_ == ControlState::BASE_CONTROL && are_all_motors_operation_enabled();
    const auto now = std::chrono::steady_clock::now();
    if (!all_motors_on) {
      motors_all_on_since_ = std::chrono::steady_clock::time_point::min();
      return;
    }
    if (motors_all_on_since_ == std::chrono::steady_clock::time_point::min()) {
      motors_all_on_since_ = now;
      return;
    }
    if ((now - motors_all_on_since_) < std::chrono::milliseconds(kHeartbeatStableMs)) {
      return;
    }
    if (last_heartbeat_publish_time_ != std::chrono::steady_clock::time_point::min() &&
      (now - last_heartbeat_publish_time_) < std::chrono::milliseconds(kHeartbeatPeriodMs)) {
      return;
    }
    std_msgs::msg::Bool heartbeat_msg;
    heartbeat_msg.data = true;
    motor_heartbeat_pub_->publish(heartbeat_msg);
    last_heartbeat_publish_time_ = now;
  }
    ...
    switch (current_state_) {
      ...
      case ControlState::MOTOR_STATE_CHECK:
        ...
        for (size_t i = 0; i < this->statuswords_.size(); ++i) {
          ...
          uint16_t controlword = 0, error_code = 0, error_reg = 0;
          ...
          if (sdo_read_u32(motor_ids_[i], 0x60FD, 0x00, digital_inputs)) {
            RCLCPP_INFO(rclcpp::get_logger("KincoHardwareInterface"), "  Motor %d digital_inputs=0x%08x", motor_ids_[i], digital_inputs);
          }
          log_din1_enable_diagnostics(motor_ids_[i], false);
      ...
      case ControlState::WAIT_STATUSWORD_ENABLED:
        ...
        if (state_step_timer_ > 250) {  // ~5 seconds at 50Hz
          ...
          for (size_t i = 0; i < this->motor_ids_.size(); ++i) {
            ...
            if (sdo_read_u32(motor_ids_[i], 0x60FD, 0x00, digital_inputs)) {
              RCLCPP_ERROR(rclcpp::get_logger("KincoHardwareInterface"), "    digital_inputs=0x%08x", digital_inputs);
            }
            log_din1_enable_diagnostics(motor_ids_[i], true);
          }
  ...
  bool sdo_read_u16(uint8_t motor_id, uint16_t index, uint8_t subindex, uint16_t & out_val, double timeout_s = 0.2) {
    ...
    while (true) {
      ...
      if ((frame.can_id & 0x7FF) != static_cast<uint32_t>(0x580 + motor_id) || frame.can_dlc != 8) {
        continue;
      }
      ...
      if (frame.data[0] == 0x80) {
        ...
        RCLCPP_WARN(
          rclcpp::get_logger("KincoHardwareInterface"),
          "SDO read abort for id=%u index=0x%04x sub=0x%02x abort=0x%08x",
          motor_id, index, subindex, abort_code);
        return false;
      }
    ...
    RCLCPP_WARN(
      rclcpp::get_logger("KincoHardwareInterface"),
      "SDO read timeout for id=%u index=0x%04x sub=0x%02x",
      motor_id, index, subindex);
    return false;
  }

  bool sdo_read_u32(uint8_t motor_id, uint16_t index, uint8_t subindex, uint32_t & out_val, double timeout_s = 0.2) {
    uint8_t req[8] = {0x40, static_cast<uint8_t>(index & 0xFF), static_cast<uint8_t>((index >> 8) & 0xFF), subindex, 0, 0, 0, 0};
    send_sdo(motor_id, req);
    struct can_frame frame{};
    const double start = static_cast<double>(std::chrono::duration_cast<std::chrono::microseconds>(
  ...
  void log_din1_enable_diagnostics(uint8_t motor_id, bool as_error) {
    uint16_t din1_function = 0;
    uint16_t input_status = 0;
    uint16_t input_polarity = 0;

    const bool got_din1_function = sdo_read_u16(motor_id, 0x2010, 0x03, din1_function);
    const bool got_input_status = sdo_read_u16(motor_id, 0x2010, 0x0A, input_status);
    const bool got_input_polarity = sdo_read_u16(motor_id, 0x2010, 0x01, input_polarity);

    if (!(got_din1_function || got_input_status || got_input_polarity)) {
      return;
    }

    bool din1_raw = false;
    bool din1_active = false;
    bool din1_active_high = true;

    if (got_input_status) {
      din1_raw = (input_status & 0x0001) != 0;
      din1_active = din1_raw;
    }
    if (got_input_polarity && got_input_status) {
      din1_active_high = (input_polarity & 0x0001) != 0;
      din1_active = din1_active_high ? din1_raw : !din1_raw;
    }

    if (as_error) {
      RCLCPP_ERROR(
        rclcpp::get_logger("KincoHardwareInterface"),
        "    Motor %u DIN1 diag: function=0x%04x input_status=0x%04x polarity=0x%04x din1_raw=%u din1_active=%u",
        motor_id, din1_function, input_status, input_polarity, din1_raw ? 1U : 0U, din1_active ? 1U : 0U);
    } else {
      RCLCPP_INFO(
        rclcpp::get_logger("KincoHardwareInterface"),
        "  Motor %u DIN1 diag: function=0x%04x input_status=0x%04x polarity=0x%04x din1_raw=%u din1_active=%u",
    ...
    // On this drive profile 0x0001 typically maps DIN1 as "Enable/Drive ON".
    if (got_din1_function && din1_function == 0x0001 && got_input_status && !din1_active) {
      if (as_error) {
        RCLCPP_ERROR(
          rclcpp::get_logger("KincoHardwareInterface"),
          "    Motor %u: DIN1 is configured as ENABLE (0x2010:03=0x0001) but inactive. Drive cannot reach Operation Enabled.",
          motor_id);
      } else {
        RCLCPP_WARN(
          rclcpp::get_logger("KincoHardwareInterface"),
          "  Motor %u: DIN1 is configured as ENABLE (0x2010:03=0x0001) but inactive.",
          motor_id);
      }
    }
  }
  ...
  void init_node2_sequence() {
    ...
    // 4) Fault reset
    {
      uint8_t cmd[] = {0x2B, 0x40, 0x60, 0x00, 0x80, 0x00, 0x00, 0x00};
      send_sdo(node_id, cmd);
    }
    usleep(200000);

    // 5) Set mode of operation to -3 (0xFD)
    {
      uint8_t cmd[] = {0x2F, 0x60, 0x60, 0x00, 0xFD, 0x00, 0x00, 0x00};
      send_sdo(node_id, cmd);
      RCLCPP_INFO(rclcpp::get_logger("KincoHardwareInterface"), "Node2: mode of operation set to -3 (0xFD)");
    }
    usleep(200000);

    // 6) Shutdown
    {
      uint8_t cmd[] = {0x2B, 0x40, 0x60, 0x00, 0x06, 0x00, 0x00, 0x00};
      send_sdo(node_id, cmd);
    }
    usleep(200000);
    ...
    // 7) Switch on
    {
      uint8_t cmd[] = {0x2B, 0x40, 0x60, 0x00, 0x07, 0x00, 0x00, 0x00};
      send_sdo(node_id, cmd);
    }
    usleep(200000);
    ...
    // 8) Enable operation
    {
      uint8_t cmd[] = {0x2B, 0x40, 0x60, 0x00, 0x0F, 0x00, 0x00, 0x00};
      send_sdo(node_id, cmd);
    }
    usleep(200000);
    ...
    if (sdo_read_u16(node_id, 0x6041, 0x00, statusword)) {
      RCLCPP_INFO(rclcpp::get_logger("KincoHardwareInterface"), "Node2: statusword = 0x%04x", statusword);
    } else {
      RCLCPP_WARN(rclcpp::get_logger("KincoHardwareInterface"), "Node2: statusword read failed");
    }

    // 10) Read controlword
    uint16_t controlword = 0;
    if (sdo_read_u16(node_id, 0x6040, 0x00, controlword)) {
      RCLCPP_INFO(rclcpp::get_logger("KincoHardwareInterface"), "Node2: controlword = 0x%04x", controlword);
    } else {
      RCLCPP_WARN(rclcpp::get_logger("KincoHardwareInterface"), "Node2: controlword read failed");
    }

    RCLCPP_INFO(rclcpp::get_logger("KincoHardwareInterface"), "Node2 init sequence complete.");
  }
  ...
  void reset_fault_seq(uint8_t motor_id) {
    ...
    sdo_write_u16(motor_id, 0x6040, 0x00, 0x0080);
    usleep(100000);

    // 2) Shutdown.
    sdo_write_u16(motor_id, 0x6040, 0x00, 0x0006);
    usleep(100000);

    // 3) Kinco-specific: toggle 0x2010:01 bit3 if available.
    uint16_t cur = 0;
    if (sdo_read_u16(motor_id, 0x2010, 0x01, cur)) {
      const uint16_t set = static_cast<uint16_t>(cur | (1 << 3));
      sdo_write_u16(motor_id, 0x2010, 0x01, set);
      usleep(50000);
      sdo_write_u16(motor_id, 0x2010, 0x01, cur);
      usleep(50000);
      RCLCPP_INFO(
        rclcpp::get_logger("KincoHardwareInterface"),
        "Motor %u: 0x2010:01 toggled (cur=0x%04x)",
        motor_id, cur);
    } else {
      RCLCPP_WARN(
        rclcpp::get_logger("KincoHardwareInterface"),
        "Motor %u: 0x2010:01 read timeout/abort, continuing",
        motor_id);
    }
  ...
  static constexpr int kHeartbeatStableMs = 500;
  static constexpr int kHeartbeatPeriodMs = 1000;
  ...
  rclcpp::Node::SharedPtr heartbeat_node_;
  rclcpp::Publisher<std_msgs::msg::Bool>::SharedPtr motor_heartbeat_pub_;
  std::chrono::steady_clock::time_point motors_all_on_since_ = std::chrono::steady_clock::time_point::min();
  std::chrono::steady_clock::time_point last_heartbeat_publish_time_ = std::chrono::steady_clock::time_point::min();
};
